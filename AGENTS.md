...
- `kubernetes/apps/` has NO root `kustomization.yaml`; the Flux kustomize-controller auto-generates one and reconciles the namespace subdirectories recursively. New namespace folders are picked up by `cluster-apps` simply by existing — do not add an apps-root kustomization, and note plain `kustomize build kubernetes/apps` fails locally while Flux/flux-local succeed.
