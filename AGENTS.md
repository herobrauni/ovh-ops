...
- For media namespace remote mounts backed by Decypharr WebDAV, the shared rclone `StorageClass` and `decypharr-rclone` PVC now live under `kubernetes/apps/media/rclone`. Media consumers should mount that shared claim rather than embedding inline CSI rclone volumes, while Decypharr itself should use its built-in WebDAV mounting instead of consuming the shared rclone PVC.
