...
- The AlteredParadox my-idlers image is not a drop-in for the original's security context. It runs supervisord as root and drops nginx/php-fpm workers to their own users, so `drop: ["ALL"]` alone breaks it; it needs `CHOWN, SETUID, SETGID, DAC_OVERRIDE, FOWNER` added back. It also refuses to boot without `APP_KEY` and needs `TRUSTED_PROXIES` set (Envoy terminates TLS; without it signed YABS ingest URLs fail validation) and a `Host: <APP_URL host>` header on HTTP probes (TrustHosts 400s a bare-IP probe against a healthy app). `idlers2` derives its `APP_KEY` deterministically from `MYSQL_ROOT_PASSWORD` in the ExternalSecret template rather than storing a second Infisical folder — rotating that password therefore rotates the key and invalidates sessions, which is the only thing it protects here (API tokens are hashed, not encrypted).
