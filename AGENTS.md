...
- `talos/`: Talos cluster config for topf (`topf.yaml`, `secrets.sops.yaml`) with patch directories (`all/`, `control-plane/`, `worker/`).
...
- Prefer `https://k8s-schemas.home-operations.com/` for Kubernetes and CRD `yaml-language-server` schema URLs wherever that site serves the needed schema; only fall back to other schema sources when no equivalent exists (e.g. `json.schemastore.org/kustomization` for Kustomize configs). The retired `kubernetes-schemas.brauni.dev` mirror must not be referenced anymore.
...
- `flux-system/konflate` renders PR diffs in-cluster. It is deliberately single-instance (`strategy: Recreate`, `replicaCount` 0 or 1 only — a second replica wedges on the RWO cache PVC) and its `/hooks` HTTPRoute is intentionally NOT behind TinyAuth: konflate verifies the GitHub webhook HMAC itself via `KONFLATE_WEBHOOK_SECRET`, and GitHub cannot complete an extAuth redirect. The UI/API route is TinyAuth-protected. Auth is GitHub App based (`config.appClientId` + `/konflate/KONFLATE_APP_PRIVATE_KEY` in Infisical — the same CI bot App the runners use; installation tokens cover reads, `Konflate` status checks and PR comments, so no PATs are stored). PR re-renders are instant once the repo webhook points at `https://konflate.brauni.dev/hooks`, otherwise the 15m `refreshInterval` backstop applies.
...
- `talos:generate-config` renders configs with `topf render`; day-2 Talos operations run through topf (`task talos:apply`, `task talos:diff`, `task talos:upgrade`, `task talos:reset`). topf intentionally does not manage Kubernetes upgrades; `task talos:upgrade-k8s` shells out to `talosctl upgrade-k8s` reading versions from `talos/topf.yaml`.
- Talos config generation was migrated from talhelper (archived) to topf (postfinance/topf) in 2026-09, following onedr0p/cluster-template's structure: `topf.yaml` + `secretsPath: secrets.sops.yaml` + per-node `data:` consumed by Go-templated `.yaml.tpl` patches instead of `node/<host>/` directories. The Talos v1.14 upgrade moved the patches to the multi-document config format (typed `Kube*Config`, `SysctlConfig`, `ResolverConfig`, `CRICustomizationConfig`, `UnattendedInstallConfig` documents). Notes: (1) a 1.14-format config is rejected by 1.13 nodes (`"UnattendedInstallConfig": not registered`), so the Talos upgrade must complete before `topf apply` converges the migrated config; (2) `topf`'s 1.14 base does NOT disable kube-proxy (its 1.13 base did) — `control-plane/00-cluster.yaml` must keep `KubeProxyConfig enabled: false` for the Cilium kube-proxy replacement; (3) `machine.kubelet.disableManifestsDirectory` has no typed-config equivalent and was dropped (cluster-template does the same); (4) `SecurityProfileConfig workloadIsolation: false` is pinned explicitly because topf's 1.14 base defaults to `true`, which would break KubeVirt/Rook/node-exporter host access.
- CoreDNS in this cluster is Helm/Flux-managed (`kubernetes/apps/kube-system/coredns`), so the machine config must keep `KubeCoreDNSConfig enabled: false`. Talos' own CoreDNS manifest uses a bare `k8s-app: kube-dns` selector while the Helm Deployment's selector also carries `app.kubernetes.io/*` labels; Deployment selectors are immutable, so `upgrade-k8s`' manifests phase fails permanently against the Helm deployment (seen during the 2026-09-05 v1.37.0 upgrade). Same reasoning as kube-proxy: Talos must not reconcile deployments it does not own.
- A tuppr `KubernetesUpgrade`/`TalosUpgrade` that exhausts its job backoffLimit goes `Failed` terminally and will not retry on its own. After fixing the underlying cause, retrigger with `kubectl -n system-upgrade annotate kubernetesupgrade kubernetes tuppr.home-operations.com/reset="$(date)"` (same for `talosupgrade`); the re-run is a no-op fast-path when the actual node work already completed.
...
- Kubernetes/CRD schemas come from the shared upstream mirror `k8s-schemas.home-operations.com` (generated from pinned upstream CRD releases plus core kinds extracted from a kind cluster's OpenAPI; new CRDs are added by opening a `sources/<owner>/<repo>/vendir.yml` PR in `home-operations/k8s-schemas`). The retired self-published `kubernetes-schemas.brauni.dev` mirror (`.github/workflows/schemas.yaml` + the `schemas-runner`) was switched off 2026-09; the Cloudflare Pages project itself can be deleted from the dashboard. Known coverage gap: `volsync.backube/kopiamaintenance` is not published upstream yet — its manifest has no modeline and is silently skipped by yayamlls (contribute a vendir source upstream, or it disappears with the eventual kopiur migration).
...
- KubeVirt uses `spec.domain.firmware.uuid` as the cloud-init `instance-id` (`cloudInitUUIDFromVMI` in `pkg/cloud-init/cloud-init.go`). If a VM has a `firmware` block without an explicit `uuid`, virt-api pins a generated one, every restart reuses the same instance-id, and cloud-init silently skips all per-instance modules (users, ssh keys, packages, runcmd) — changed user-data never applies. Always set `firmware.uuid` explicitly and bump it to a fresh UUID whenever cloud-init user-data changes. Symptom seen here: hostname from meta-data applied, ssh key rejected, no `AgentConnected` condition, while the same image + noCloud.iso boots correctly under local qemu from a fresh disk.
- Guests with masquerade networking are reached through the virt-launcher pod IP (inbound DNAT to 10.0.2.15). `virtctl ssh`'s embedded golang ssh client rejects valid keys where the real `ssh` binary succeeds — use `kubectl port-forward` to the launcher pod (or a Service selecting `kubevirt.io/domain: <vm>`) plus the real ssh client instead. The cluster pod network is IPv4-only, so guests get no routable IPv6 (only `fec0::` site-local auto-addrs); use Tailscale inside a guest if tailnet v6 is needed.
- KubeVirt and CDI are installed from their vendored upstream operator manifests under `kubernetes/apps/kubevirt` and `kubernetes/apps/cdi`; there is no official upstream Helm chart. Runtime images are overridden with digest-pinned operator environment variables. When Renovate bumps only the component images (`operator/images.yaml`), the vendored RBAC can lag the operator version: a new KubeVirt minor may grant itself permissions its own vendored ClusterRole does not hold (e.g. v1.9.0 introduced `plugin.kubevirt.io`), and the API server's RBAC escalation guard then rejects the update with `is forbidden: ... attempting to grant RBAC permissions not currently held`, stalling the upgrade as `DeploymentFailed`. Fix by re-vendoring `operator.yaml` from the matching upstream release (drop the leading Namespace doc, keep the vendored-from header, run through `yamlfmt` with the repo `.yamlfmt.yaml`); a version mismatch between the vendored file header and the image env vars is the tell-tale sign. VM disks intended for live migration should use the virtualization-default `ceph-block-kubevirt` StorageClass and CDI StorageProfile, which provision RWX raw RBD block volumes with only the `layering` image feature and a `Retain` reclaim policy.
...
- If `helm pull` against `ghcr.io` fails with `403 denied` on the token endpoint, the cause is a stale/insufficient credential in `~/.docker/config.json`, not a network block. ghcr returns 403 rather than falling back to anonymous when it is sent a bad token. Confirm with `curl -s -o /dev/null -w '%{http_code}' 'https://ghcr.io/token?scope=repository:bjw-s-labs/helm/app-template:pull&service=ghcr.io'` (200 = network fine), then re-run with a valid token via an isolated `DOCKER_CONFIG` dir. A working token lives in `~/.config/github/GH_TOKEN`. This does not affect flate (single Go binary, no docker config involved).
- `kubernetes/apps/` has NO root `kustomization.yaml`; the Flux kustomize-controller auto-generates one and reconciles the namespace subdirectories recursively. New namespace folders are picked up by `cluster-apps` simply by existing — do not add an apps-root kustomization, and note plain `kustomize build kubernetes/apps` fails locally while Flux/flate succeed.
- The EU Infisical host differs from the public API docs: Universal Auth login is `POST /api/v1/auth/universal-auth/login` (body `clientId`/`clientSecret`), reads are `GET /api/v3/secrets/raw?workspaceId=<project-id>&environment=prod&secretPath=...`, and plaintext writes are `POST /api/v4/secrets/<key>` (body `projectId`/`environment`/`secretPath`/`secretValue`). The v3 write endpoints demand KMS ciphertext; a folder must exist before secrets can be created inside it (`POST /api/v1/folders`). The project ID for slug `ovh-ops-u1ua` comes from `GET /api/v1/projects`. Plain python urllib requests get Cloudflare 1010-blocked — send a browser-like `User-Agent`.
- LiteLLM hardened (non-root) deployments MUST use the `ghcr.io/berriai/litellm-non_root` image variant — the standard `litellm` image expects to run as root and bakes no `PRISMA_*` env, so under `runAsNonRoot` + `readOnlyRootFilesystem` the python prisma client can't resolve its query engine and the proxy crash-loops with `NotConnectedError` (while `prisma migrate deploy` still succeeds, masking it). The `litellm-database` tag series is legacy. The non_root variant is read-only-rootfs + arbitrary-UID capable when you replicate upstream's hardened compose: emptyDirs at `/app/cache` and `/app/migrations` plus `HOME=/tmp`, `XDG_CACHE_HOME=/app/cache`, `LITELLM_MIGRATION_DIR=/app/migrations`, `LITELLM_NON_ROOT=true`; prisma engines are baked readable at `/opt/prisma` with `PRISMA_OFFLINE_MODE=true`. Probes: liveness `/health/liveliness`, readiness/startup `/health/readiness` (startup needs a generous failureThreshold — first boot runs prisma migrations). `STORE_MODEL_IN_DB=True` allows adding models via the UI while Git only holds a minimal `config.yaml` anchor.
 - LiteLLM gets its Redis-compatible store from the shared `kubernetes/components/dragonfly` component (`litellm-dragonfly.ai.svc.cluster.local:6379`), same pattern as open-webui/paperless. Redis is wired through `litellm_settings.cache_params` in the embedded proxy `config.yaml` (`type: redis`, `namespace: litellm`, `ttl: 300`, `port` as quoted string) plus `enable_redis_auth_cache: true` so virtual-key auth lookups share the cache across workers/future replicas. Verify the live cache with an authenticated `GET /cache/ping` on port 4000.
 - Renovate: version Talos/Kubernetes bumps with `datasource=github-releases` (`siderolabs/talos`, `kubernetes/kubernetes`) in `talos/topf.yaml` + `kubernetes/apps/system-upgrade/tuppr/upgrades/*`, NOT `datasource=docker` on `ghcr.io/siderolabs/{installer,kubelet}`. The Sidero OCI manifests carry no timestamp labels, so any `minimumReleaseAge` (default `minimumReleaseAgeBehaviour: timestamp-required`) plus `RENOVATE_INTERNAL_CHECKS_FILTER: strict` in the Renovate workflow silently blocks every update forever — the dep sits in the dashboard's "Pending Status Checks" section with `Branch ... creation is disabled because internalChecksFilter was not met` in the debug logs. Cooldown policy: 3 days for Talos/Kubernetes versions (`.renovate/groups.json5`), 7 days for tools (mise, GitHub Actions), none for in-cluster app images. Do not reference undefined custom datasources (a `custom.talos-factory` annotation was dead code and killed Talos updates until removed). Renovate runs self-hosted via `.github/workflows/renovate.yaml`; its debug logs (`gh api repos/herobrauni/ovh-ops/actions/runs/<id>/logs`) are the fastest way to diagnose "why is X not updating".
- Authentik is the planned TinyAuth replacement for Envoy Gateway `extAuth` (cut over for `sonarr.480p.com`, `radarr.480p.com`, `sonarr4k.480p.com` and `radarr4k.480p.com` first). Forward auth is served by the **embedded outpost in the `authentik-server` process** — there is no separate `ak-outpost-*` Service/Deployment — so `SecurityPolicy.extAuth.http.backendRefs` points at Service `authentik-server` (ns `authentik`, port **80** → container 9000), path `/outpost.goauthentik.io/auth/envoy`, with `headersToBackend: [Location, Set-Cookie, X-authentik-*]` (same pass-through shape TinyAuth needed; EG v1.9.1 forwards both). Unlike the older Go outpost, the 2026.8 outpost is OIDC-based: `/auth/envoy` 302s to `https://sso.brauni.dev/application/o/authorize/?client_id=<provider>&redirect_uri=<app>/outpost.goauthentik.io/callback?X-authentik-auth-callback=true`, so **every protected hostname also needs its own HTTPRoute for `PathPrefix /outpost.goauthentik.io` → `authentik-server:80`** (must be a *separate* HTTPRoute so the app's SecurityPolicy does not ext-auth the callback into a redirect loop) plus a `ReferenceGrant` in ns `authentik` covering SecurityPolicy/HTTPRoute from the consumer namespace (`kubernetes/apps/authentik/authentik/app/referencegrant.yaml` is scoped to `media`; add namespaces as apps migrate). Provider/Application objects are **not** in Git yet (created via the API/UI): `forward_single` mode, `external_host` = the app URL, assigned to the `authentik Embedded Outpost`, `cookie_domain` empty (the `authentik_proxy_*` cookie is host-only and cannot span `brauni.dev`/`riki.boo`/`480p.com` — there is no common registrable domain, so per-app login is expected and the shared `sso.brauni.dev` session makes the second app a silent redirect). Provider matching is by `Host`, so the original `:authority` must survive to the ext-auth request (it does). A raw ORM create must call `provider.set_oauth_defaults()` (grant_types, callback `redirect_uris`, proxy scope mapping) or the provider is incomplete — the UI/API serializer does this automatically. Known upstream wart: envoy-mode state JWTs carry a protocol-relative post-login `redirect` (`//sonarr.480p.com/`, goauthentik/authentik#19653, closed as not planned) which desktop browsers resolve fine but mobile Chrome/Brave have been reported to mishandle. Application access is enforced by `PolicyEngine` bindings on the Application, not by authentication alone: `core_default_app_access=True` only makes an app open while it has **zero** bindings, so binding anything (here a `PolicyBinding` → Group `Media`, allow, order 0) locks out every other authenticated user — and `PolicyAccessView.user_has_access()` has **no superuser bypass**, so `akadmin` is denied too until it is in the group. The Sonarr/Radarr providers, Applications, the `Media` group, its two bindings and the `brauni`/`authentik_testuser1` users were created by hand through the API/UI and are not in Git yet. Cut-over since: `sonarr4k`/`radarr4k`, then `altmount` (`skip_path_regex: ^/health$`), `clonarr` (`^/api/health$`), `prowlarr` (`^/ping$`) and `umlautadaptarrex` (`^/api/health$`). Step-by-step runbook for migrating further apps (exact `ak shell` objects, manifest diffs, curl verification recipe, rollback): `docs/authentik-forward-auth.md`.
- Gatus endpoints behind Authentik must probe a liveness path, because an unauthenticated probe of `/` now gets a 302 instead of 200. There is no single such path: `*arr` apps (sonarr/radarr/prowlarr, incl. 4K) serve `/ping` anonymously, but altmount only answers `/health` (`^/health$`), and clonarr and umlautadaptarrex only answer `/api/health` (`^/api/health$`). An in-cluster probe from a throwaway pod finds candidates but is not proof (apps may exempt local addresses); confirm the path through the gateway without credentials after the migration. If an app has no anonymous path, either assert its own auth challenge (`gatus.home-operations.com/endpoint: |- conditions: ["[STATUS] == 401"]`, as altmount's `/webdav` route does) or disable that endpoint. Per-app liveness paths are tabulated in `docs/authentik-forward-auth.md`. Otherwise: set `skip_path_regex` on the ProxyProvider, override the URL per route with `gatus.home-operations.com/endpoint: |- url: https://<app>.480p.com/<path>`, and set `gatus.home-operations.com/enabled: "false"` on the `*-outpost` HTTPRoute — otherwise `gatus-sidecar --auto-httproute` also emits a bogus endpoint for `https://<app>.480p.com/outpost.goauthentik.io`, since it derives one endpoint per HTTPRoute and deep-merges the parent Gateway's annotation into the child (child wins on scalars). The outpost evaluates `skip_path_regex` **before** basic auth, so the Gateway's inherited `Authorization: Basic ${GATUS_TINYAUTH_BASIC_AUTH}` header is harmless on those probes and can stay until TinyAuth is fully retired.
- `gatus-sidecar --auto-httproute` derives each endpoint's probe **URL from the HTTPRoute rule path**, not from the hostname root, and `--enable-service` likewise — so a route whose first rule is a path-specific workaround gets probed there. Plex was red for a long time because its route's first rule is `PathPrefix /library/streams` (a `Range`-header strip for direct streaming, which newer PMS 404s); an explicit `url:` in the `gatus.home-operations.com/endpoint` annotation is the fix (`plex` now probes `/identity`, PMS' anonymous identity endpoint). Routes whose handler rejects GET need a matching condition instead of an `== 200`: the POST-only webhook receivers (Flux `github-webhook`, konflate `/hooks`) both assert `conditions: ["[STATUS] == 404"]`, which the app answers with Go's own `404 page not found` — do not switch those to a POST probe, konflate logs a `WARN webhook verification failed` per attempt and an unsigned payload is rejected by design anyway.

## Testing Guidelines
Primary validation is CI-based:
- `.github/workflows/flate.yaml` runs flate `test` (full-tree validation) and `diff` (rendered, GitHub-flavored PR comment) for PRs touching `kubernetes/**`. flate replaced the deprecated flux-local; the local equivalent of the CI gate is `flate test all -p ./kubernetes/flux/cluster` (single static binary, renders the whole repo in seconds).
