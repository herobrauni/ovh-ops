...
- Authentik was moved from the long-lived `gh-sdko-oauth-account-selection` development image to official `2026.8.0-rc6` on 2026-08-05. Because the development image had applied rewritten versions of the user-switching migrations, the existing equivalent schemas were verified and official migrations `authentik_brands.0015_brand_flow_user_switch` and `authentik_core.0062_userswitchingsession_and_more` were fake-applied once before the remaining RC6 migrations ran normally. Do not delete or reapply those migration records; they reconcile the database history with official releases.
