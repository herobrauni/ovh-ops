...
- Talos config generation was migrated from talhelper (archived) to topf (postfinance/topf) in 2026-09, following onedr0p/cluster-template's structure: `topf.yaml` + `secretsPath: secrets.sops.yaml` + per-node `data:` consumed by Go-templated `.yaml.tpl` patches instead of `node/<host>/` directories. The cluster stays on the Talos v1.13 single-document config format (`machine:`/`cluster:` maps) — the v1.14 multi-document config migration (typed `Kube*Config` kinds) is deliberately deferred to a separate change. Expected first-apply diff: `machine.install.image` changes from `factory.talos.dev/installer/<schem>` to `factory.talos.dev/metal-installer/<schem>` (same image, current factory repo alias) with the current `talosVersion` tag — safe, only used at install/upgrade time.
