...
- Authentik was moved from the long-lived `gh-sdko-oauth-account-selection` development image to official `2026.8.0-rc6` on 2026-08-05. Because the development image had applied rewritten versions of the user-switching migrations, the existing equivalent schemas were verified and official migrations `authentik_brands.0015_brand_flow_user_switch` and `authentik_core.0062_userswitchingsession_and_more` were fake-applied once before the remaining RC6 migrations ran normally. Do not delete or reapply those migration records; they reconcile the database history with official releases. The public tenant's `setup` flag also remained false despite a completed setup flow and populated instance, so it was corrected once with `manage set_flag setup true --schema public`; otherwise `/` redirected to the non-applicable `/setup` flow instead of the login flow.
...
- Tailnet-only apps intentionally skip the "App Exposure Rule" (`DNSEndpoint` + Homepage annotations). That rule presumes a public Envoy route on `brauni.dev`; apps exposed solely through the tailscale-operator get their name from MagicDNS and have no public route to annotate. Layer 3 uses `spec.type: LoadBalancer` + `spec.loadBalancerClass: tailscale` on the Service (app-template v5 templates all three fields); layer 7 uses a standalone Ingress with `ingressClassName: tailscale` and `spec.tls[].hosts`. First consumers: `kubernetes/apps/obsidian/syncthing` and `kubernetes/apps/obsidian/obsidian-web`.
- pgvector's `vector` extension is **not** trusted, so the unprivileged role that `ghcr.io/home-operations/postgres-init` creates cannot `CREATE EXTENSION` itself. Install it into `template1` via `Cluster.spec.bootstrap.initdb.postInitTemplateSQL` — `postgres-init` calls `createdb --owner` without `--template`, so it defaults to `template1` and every new database inherits the extension. See `kubernetes/apps/dbms/cloudnative-pg/cluster/obsidian.yaml`. Do not add a `Database` CR for this; nothing else in this repo uses one.
- The `obsidian` CNPG cluster deliberately has no `backup:` block. It holds only a retrieval index that is rebuildable from the vault markdown, so backing it up would be storing derived state.
- Hermes Agent's `state.db` is SQLite in WAL mode and must stay on a block-backed RWO volume (`ceph-block`), never CephFS.
